...
- status: code done (2026-07-09, same PR as M3-5; `registrar_decision` implemented as the credential-free demo capability, CI-runnable e2e test proves prompt -> tool call -> local audit entry -> `GET /api/status` returns it). REMAINING for full acceptance: the live run against the deployed dogfood society (redeploy it on this build, configure a model key via M3-1, POST /api/agent with a decision prompt, see it in the studio cockpit's "Acciones recientes"). Steps documented in apps/sociedad-ia-starter/README.md ("prueba en vivo").
- priority: P1
