...
- status: blocked (all sub-items M1-4a..M1-4d are code-complete as of 2026-07-12; now gated only on a live end-to-end run against a non-production target, which needs a running studio plus a live model key, and on the escalated publish/naming decision below. Not offline-verifiable, so a loop run cannot close it.)
- priority: P2
