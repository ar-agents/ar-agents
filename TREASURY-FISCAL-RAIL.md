...
8. 🔄 **Remaining:** LIVE runs against real accounts.
   ...
   - ⏭️ **Off-ramp LIVE run** needs a provider account, but the gate is now self-driven, not a sales "yes". **Chosen path: Mural** (muralpay.com) — a stablecoin->fiat payout API with full USDC(Base)->ARS-to-CBU/CVU, self-onboard KYB (no sales gate). `MuralOffRampAdapter` is built from Mural's OpenAPI spec (`/payouts/fees/token-to-fiat` quote, `/payouts/payout` create + `/execute` commit, `/payouts/payout/{id}` status; Bearer + `transfer-api-key`; ARS rail with `bankAccountNumberType` CVU/CBU/ALIAS) and **wire-verified against the real sandbox** (`api-staging.muralpay.com` returns the exact `401 Authentication failed` the adapter handles). Three alternatives evaluated (Bridge/Stripe, Conduit, Koywe) — Mural wins for self-driven + LatAm-payout-native + best FX. Provider-optional: Ripio B2B (sandbox wire-verified) + Manteca remain as the scale/partnership layer. `scripts/live-offramp.mjs mural` makes it one command once KYB issues keys (offline full-loop demo with none). Wired into the generated society (`getOffRamp()` tries Mural -> Ripio -> Manteca; `MURAL_*` env).
