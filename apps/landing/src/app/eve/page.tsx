import type { Metadata } from "next";
import type { CSSProperties, ReactNode } from "react";
import { EveDemo } from "./eve-demo";

const FONT_MONO = "var(--font-geist-mono), ui-monospace, monospace";
const REPO = "https://github.com/ar-agents/ar-agents";
const APP = `${REPO}/tree/main/apps/incorporate-agent`;

export const metadata: Metadata = {
  title: "Built on eve",
  description:
    "ar-agents runs on eve, Vercel's open-source agent framework. An agent that incorporates an Argentine Sociedad Automatizada and pauses for a human to approve the irreversible step, art. 102 as one line: needsApproval: always().",
  alternates: { canonical: "https://ar-agents.ar/eve" },
  openGraph: {
    title: "ar-agents, built on eve",
    description:
      "An agent that incorporates an Argentine company and stops for a human to sign. Art. 102 in one line of eve: needsApproval: always().",
    url: "https://ar-agents.ar/eve",
    type: "article",
  },
};

// --- small primitives, tuned to the site's Geist + CSS-var design system ---

function Section({
  children,
  style,
  id,
}: {
  children: ReactNode;
  style?: CSSProperties;
  id?: string;
}) {
  return (
    <section
      id={id}
      style={{ maxWidth: 880, margin: "0 auto", padding: "0 24px", ...style }}
    >
      {children}
    </section>
  );
}

function Eyebrow({ children }: { children: ReactNode }) {
  return (
    <p
      style={{
        fontFamily: FONT_MONO,
        fontSize: 12,
        textTransform: "uppercase",
        letterSpacing: "0.16em",
        color: "var(--accent)",
        fontWeight: 600,
        margin: 0,
      }}
    >
      {children}
    </p>
  );
}

function H2({ children, id }: { children: ReactNode; id?: string }) {
  return (
    <h2
      id={id}
      style={{
        fontSize: "clamp(24px, 4vw, 32px)",
        fontWeight: 600,
        lineHeight: 1.2,
        letterSpacing: "-0.03em",
        margin: 0,
        color: "var(--text)",
        scrollMarginTop: 80,
      }}
    >
      {children}
    </h2>
  );
}

function P({ children, style }: { children: ReactNode; style?: CSSProperties }) {
  return (
    <p
      style={{
        fontSize: 17,
        lineHeight: 1.65,
        color: "var(--text-body)",
        margin: 0,
        maxWidth: 660,
        ...style,
      }}
    >
      {children}
    </p>
  );
}

function Mono({ children }: { children: ReactNode }) {
  return (
    <code style={{ fontFamily: FONT_MONO, color: "var(--accent)", fontSize: "0.92em" }}>
      {children}
    </code>
  );
}

/** Editor-style code window: filename header bar + body. */
function CodeWindow({
  file,
  children,
}: {
  file: string;
  children: ReactNode;
}) {
  return (
    <div
      style={{
        background: "var(--bg-tint)",
        borderRadius: 10,
        boxShadow: "var(--card-shadow)",
        overflow: "hidden",
      }}
    >
      <div
        style={{
          display: "flex",
          alignItems: "center",
          gap: 8,
          padding: "9px 14px",
          boxShadow: "inset 0 -1px 0 var(--border-color)",
        }}
      >
        <span
          style={{
            fontFamily: FONT_MONO,
            fontSize: 11.5,
            color: "var(--text-muted)",
            letterSpacing: "0.02em",
          }}
        >
          {file}
        </span>
      </div>
      <pre
        style={{
          margin: 0,
          padding: "16px 18px",
          overflowX: "auto",
          fontFamily: FONT_MONO,
          fontSize: 13,
          lineHeight: 1.7,
          color: "var(--text)",
        }}
      >
        {children}
      </pre>
    </div>
  );
}

// Line helpers for the code windows.
const cm: CSSProperties = { color: "var(--text-muted)" }; // comment / punctuation
const kw: CSSProperties = { color: "var(--text-body)" }; // keyword-ish
const hl: CSSProperties = {
  color: "var(--accent)",
  fontWeight: 600,
  background: "var(--accent-bg)",
  borderRadius: 3,
  padding: "1px 4px",
  margin: "0 -4px",
};

function PrimaryLink({ href, children }: { href: string; children: ReactNode }) {
  return (
    <a
      href={href}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: 8,
        padding: "9px 18px",
        background: "var(--primary-bg)",
        color: "var(--primary-text)",
        borderRadius: 7,
        fontSize: 14.5,
        fontWeight: 500,
        textDecoration: "none",
        letterSpacing: "-0.01em",
      }}
    >
      {children}
    </a>
  );
}

function GhostLink({ href, children }: { href: string; children: ReactNode }) {
  return (
    <a
      href={href}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: 8,
        padding: "9px 16px",
        background: "transparent",
        color: "var(--text)",
        borderRadius: 7,
        fontSize: 14.5,
        fontWeight: 500,
        textDecoration: "none",
        boxShadow: "var(--shadow-ring-light)",
        letterSpacing: "-0.01em",
      }}
    >
      {children}
    </a>
  );
}

const USES: ReadonlyArray<readonly [string, ReactNode]> = [
  [
    "Connection (MCP)",
    <>
      <Mono>ar-agents.ar/api/mcp</Mono>. 235 Argentine tools, zero credentials. eve
      speaks MCP, so connecting took no SDK change.
    </>,
  ],
  [
    "Tools",
    <>
      <Mono>incorporar_sociedad</Mono> and <Mono>registrar_decision</Mono>, one
      TypeScript file each.
    </>,
  ],
  [
    "Approvals (HITL)",
    <>
      <Mono>needsApproval: always()</Mono> on the irreversible step. The run parks
      for a human (art. 102).
    </>,
  ],
  [
    "Skills",
    <>
      The Sociedad Automatizada rules and the AFIP/ARCA landmines, plain markdown,
      loaded on demand.
    </>,
  ],
  [
    "Evals",
    <>
      A scored test that the run pauses for approval instead of constituting a
      company unattended.
    </>,
  ],
  [
    "Durable execution",
    <>
      The parked turn survives restarts and redeploys, then resumes the moment a
      person answers.
    </>,
  ],
];

export default function EvePage() {
  return (
    <main
      style={{
        background: "var(--bg)",
        color: "var(--text)",
        paddingBottom: 120,
        fontFamily: "var(--font-geist-sans), Arial, sans-serif",
      }}
    >
      {/* HERO */}
      <Section style={{ paddingTop: 72 }}>
        <Eyebrow>built on eve</Eyebrow>
        <h1
          style={{
            fontSize: "clamp(36px, 7vw, 60px)",
            fontWeight: 600,
            lineHeight: 1.04,
            letterSpacing: "-0.04em",
            margin: "20px 0 0",
            maxWidth: 760,
          }}
        >
          An agent that incorporates a company.
        </h1>
        <P style={{ fontSize: 19, marginTop: 22, maxWidth: 680 }}>
          eve is Vercel&apos;s open-source agent framework. ar-agents already speaks
          the protocol it connects over, so this agent incorporates a real Argentine
          company, and pauses for a human to approve the one step it must not take
          alone.
        </P>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 28 }}>
          <PrimaryLink href="#demo">
            Run the demo
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 5v14" /><path d="m19 12-7 7-7-7" /></svg>
          </PrimaryLink>
          <GhostLink href={APP}>
            Source
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M7 17 17 7" /><path d="M7 7h10v10" /></svg>
          </GhostLink>
        </div>
      </Section>

      {/* CONTEXT for non-Argentine readers */}
      <Section style={{ paddingTop: 28 }}>
        <p
          style={{
            fontSize: 13.5,
            lineHeight: 1.6,
            color: "var(--text-muted)",
            margin: 0,
            maxWidth: 660,
            paddingLeft: 14,
            boxShadow: "inset 2px 0 0 var(--border-color)",
          }}
        >
          A Sociedad Automatizada is the AI-operated company in Argentina&apos;s draft
          companies law. The human administrator stays liable and cannot delegate
          supervision (art. 102), and the name must carry the word{" "}
          <span style={{ fontFamily: FONT_MONO }}>Automatizada</span> (art. 14).
        </p>
      </Section>

      {/* DEMO */}
      <Section id="demo" style={{ paddingTop: 56 }}>
        <EveDemo />
        <p
          style={{
            fontSize: 12.5,
            color: "var(--text-muted)",
            margin: "12px 2px 0",
            fontFamily: FONT_MONO,
          }}
        >
          A scripted replay of the real run. Approve to continue, reject and nothing
          is constituted.
        </p>
      </Section>

      {/* ONE LINE */}
      <Section style={{ paddingTop: 96 }}>
        <Eyebrow>human-in-the-loop</Eyebrow>
        <H2 id="approval">Human approval in one line</H2>
        <P style={{ marginTop: 16, marginBottom: 28 }}>
          Art. 102 makes the human administrator liable for what the AI does, and bars
          delegating the supervision duty. So the agent validates the CUIT and drafts
          the plan on its own. The one step it cannot take alone, constituting the
          company, is gated.
        </P>
        <CodeWindow file="agent/tools/incorporar_sociedad.ts">
          <span style={kw}>import</span> {"{ defineTool }"} <span style={kw}>from</span>{" "}
          <span style={{ color: "var(--accent)" }}>&quot;eve/tools&quot;</span>;{"\n"}
          <span style={kw}>import</span> {"{ always }"} <span style={kw}>from</span>{" "}
          <span style={{ color: "var(--accent)" }}>&quot;eve/tools/approval&quot;</span>;
          {"\n\n"}
          <span style={kw}>export default</span> <span style={{ color: "var(--text)" }}>defineTool</span>({"{"}
          {"\n"}
          {"  "}description: <span style={cm}>&quot;Incorporate an Argentine company. Irreversible.&quot;</span>,{"\n"}
          {"  "}inputSchema: z.object({"{ denominacion, tipo, objeto, representante }"}),{"\n"}
          {"  "}<span style={hl}>needsApproval: always(),</span>{"\n"}
          {"  "}<span style={kw}>async</span> execute(input) {"{"} <span style={cm}>/* POST /api/auto-incorporate */</span> {"}"}{"\n"}
          {"}"});
        </CodeWindow>
        <P style={{ marginTop: 18, fontSize: 15, color: "var(--text-muted)" }}>
          eve parks the run durably until a person answers. That pause is the legal
          requirement, expressed as configuration.
        </P>
      </Section>

      {/* DIRECTORY */}
      <Section style={{ paddingTop: 96 }}>
        <Eyebrow>filesystem-first</Eyebrow>
        <H2>The whole agent is a directory</H2>
        <P style={{ marginTop: 16, marginBottom: 28 }}>
          In eve, a file&apos;s name and place in the tree are its definition. There is
          no wiring file. eve discovers the directory and compiles it into an app that
          runs on Vercel Functions.
        </P>
        <CodeWindow file="apps/incorporate-agent">
          <span style={{ color: "var(--text)" }}>agent/</span>{"\n"}
          {"  "}agent.ts            <span style={cm}>model + config</span>{"\n"}
          {"  "}instructions.md     <span style={cm}>the assistant, in Argentine Spanish</span>{"\n"}
          {"  "}connections/{"\n"}
          {"    "}<span style={{ color: "var(--accent)" }}>ar-agents.ts</span>      <span style={cm}>MCP → ar-agents.ar (235 tools)</span>{"\n"}
          {"  "}tools/{"\n"}
          {"    "}<span style={{ color: "var(--accent)" }}>incorporar_sociedad.ts</span>   <span style={cm}>needsApproval: always()</span>{"\n"}
          {"    "}registrar_decision.ts    <span style={cm}>signed audit log</span>{"\n"}
          {"  "}skills/{"\n"}
          {"    "}sociedad-automatizada.md{"\n"}
          {"    "}afip-arca-landmines.md{"\n"}
          <span style={{ color: "var(--text)" }}>evals/</span>{"\n"}
          {"  "}aprobacion-humana.eval.ts{"\n"}
          {"  "}denominacion-y-supervision.eval.ts
        </CodeWindow>
      </Section>

      {/* HOW IT USES EVE */}
      <Section style={{ paddingTop: 96 }}>
        <Eyebrow>the mapping</Eyebrow>
        <H2>How it uses eve</H2>
        <div style={{ marginTop: 28, display: "grid", gap: 1 }}>
          {USES.map(([k, v]) => (
            <div
              key={k}
              style={{
                display: "grid",
                gridTemplateColumns: "minmax(150px, 200px) 1fr",
                gap: 24,
                padding: "16px 2px",
                boxShadow: "inset 0 -1px 0 var(--border-color)",
                alignItems: "baseline",
              }}
              className="eve-use-row"
            >
              <span
                style={{
                  fontFamily: FONT_MONO,
                  fontSize: 12.5,
                  color: "var(--text)",
                  fontWeight: 500,
                  letterSpacing: "0.01em",
                }}
              >
                {k}
              </span>
              <span style={{ fontSize: 15, lineHeight: 1.6, color: "var(--text-body)" }}>
                {v}
              </span>
            </div>
          ))}
        </div>
      </Section>

      {/* ONE CONNECTION */}
      <Section style={{ paddingTop: 96 }}>
        <Eyebrow>zero glue</Eyebrow>
        <H2>Connecting ar-agents took one file</H2>
        <P style={{ marginTop: 16, marginBottom: 28 }}>
          eve reaches external tools over MCP. ar-agents publishes its 235 tools as a
          hosted MCP server, so the connection is the whole integration. No wrappers,
          no re-implementation.
        </P>
        <CodeWindow file="agent/connections/ar-agents.ts">
          <span style={kw}>import</span> {"{ defineMcpClientConnection }"} <span style={kw}>from</span>{" "}
          <span style={{ color: "var(--accent)" }}>&quot;eve/mcp&quot;</span>;{"\n\n"}
          <span style={kw}>export default</span> defineMcpClientConnection({"{"}
          {"\n"}
          {"  "}url: <span style={hl}>&quot;https://ar-agents.ar/api/mcp&quot;</span>,{"\n"}
          {"}"});
        </CodeWindow>
        <P style={{ marginTop: 18, fontSize: 15, color: "var(--text-muted)" }}>
          The agent calls CUIT validation, padrón lookups, fiscal math, and the
          incorporation endpoint through this one connection.
        </P>
      </Section>

      {/* RUN IT */}
      <Section style={{ paddingTop: 96 }}>
        <Eyebrow>run it</Eyebrow>
        <H2>Clone it and go</H2>
        <P style={{ marginTop: 16, marginBottom: 28 }}>
          eve is in public preview and needs Node 24.
        </P>
        <CodeWindow file="terminal">
          <span style={cm}>$</span> git clone {REPO.replace("https://", "")}{"\n"}
          <span style={cm}>$</span> cd ar-agents/apps/incorporate-agent{"\n"}
          <span style={cm}>$</span> pnpm install && pnpm dev
        </CodeWindow>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 28 }}>
          <PrimaryLink href={APP}>
            Read the source
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M7 17 17 7" /><path d="M7 7h10v10" /></svg>
          </PrimaryLink>
          <GhostLink href="https://vercel.com/eve">eve</GhostLink>
          <GhostLink href="/sdk">ar-agents docs</GhostLink>
        </div>
        <p
          style={{
            fontSize: 13,
            lineHeight: 1.6,
            color: "var(--text-muted)",
            margin: "40px 0 0",
            maxWidth: 640,
          }}
        >
          The regime is a draft bill in the Senate, not law yet. This is a reference
          implementation and a verifiable demo, not a registered company.
        </p>
      </Section>
    </main>
  );
}
