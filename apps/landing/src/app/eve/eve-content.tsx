...
const COPY = {
  en: {
    ...
    body_hitl: "Art. 102 makes a person liable for what the AI does, and bars delegating supervision. So the agent handles the low-risk steps on its own: it validates the CUIT and drafts the plan. The one step it can't take on its own, constituting the company, is held.",
    ...
    body_infra: "Usually an agent's infra is wired by hand: model routing, the permissions of each tool, the isolation of what the model can touch. Not here. You write the agent directory and eve produces that infra around it. What you see below we didn't configure: it falls out of how the agent is written.",
    caption_infra: "You don't declare Gateway, Connection, or Sandbox separately. You write the agent as files, and eve compiles them into the infra that runs on Vercel Functions.",
    sub_connect: "what we don't use",
    body_connect: "Vercel Connect is for delegated third-party access: a customer lends the agent its own scope of ARCA, Mercado Pago, or Slack, with short per-request runtime tokens via OIDC instead of stored secrets. It's not for the team's own credentials. Here both ends of the call are the same team, so the API-key connector would just hand us back our own key wrapped in a billed token. For this first-party call we use platform OIDC, the right primitive, with a shared secret as a bridge for now. Connect stays for when a third party delegates its scope to the agent.",
    ...
    body_tested: "That the agent never constitutes a company on its own isn't a hope. It's an eval that runs with eve eval, locally or against the deployed agent. If a prompt edit or a model swap ever lets it skip the human, the eval fails before a user would.",
...
export function EveContent() {
  ...
  const uses: ReadonlyArray<readonly [string, ReactNode]> =
    lang === "es"
      ? [
          ["Conexión (MCP)", <>
            <Mono>ar-agents.ar/api/mcp</Mono>. 243 tools argentinas, cero credenciales.
      ...
      : [
          ["Connection (MCP)", <>
            <Mono>ar-agents.ar/api/mcp</Mono>. 243 Argentine tools, zero credentials.
  ...
  const infra: ReadonlyArray<readonly [string, ReactNode]> =
    lang === "es"
      ...
      : [
          ...
          ["Sandbox", <>
            Starts <Mono>deny-all</Mono>. A model compromised by prompt injection can't
    ...
    >
      ...
      <Section style={{ paddingTop: 96 }}>
        ...
        <CodeWindow file="apps/incorporate-agent">
          ...
          {"  "}connections/{"\n"}
          {"    "}<span style={str}>ar-agents.ts</span>      <span style={cm}>MCP → ar-agents.ar (243 tools)</span>{"\n"}
          {"  "}tools/{"\n"}
