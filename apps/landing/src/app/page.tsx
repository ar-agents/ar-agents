const PACKAGES = [
  ...
  },
  {
    ...
    version: "0.10.0",
    purpose:
      "EL agente de Mercado Pago más completo posible (86 tools). v0.10 = compliance + DX + observability deepening: AuditLogger (every state-mutating call captured con operation/actor/inputHash/resourceId/outcome — financial-grade compliance trail), WebhookDedup (short-circuits las retries de MP que duplican procesamiento), pagination helpers (AsyncIterable para los 7 endpoints paginados de MP), TokenBucketRateLimiter con adaptive learning de los headers de MP, AR_ISSUER_PROMOS catalog (10+ bancos AR + Ahora program), OpenTelemetry instrumentation subpath (`/otel`), 3DS challenge resolution full flow, +4 nuevos tools (find_applicable_promos, confirm_3ds_challenge, search_payments_all, list_settlements_all). Sumás v0.9: CircuitBreaker + deadline propagation + W3C Trace Context + replay-attack protection. v0.8: Edge Runtime + Vercel KV adapters + Cookbook. **245 tests pass**, 3 subpaths (`.`, `/vercel-kv`, `/otel`), publint clean, attw 🟢. **Comparado con Stripe Agent Toolkit**: 86 tools vs 26, circuit breaker + audit log + webhook dedup + AR cuotas catalog + OTEL subpath (todo NEW vs Stripe).",
    tools: [
      "AuditLogger + WebhookDedup + pagination + rate limiter (NEW v0.10)",
      "AR_ISSUER_PROMOS catalog + find_applicable_promos tool (NEW v0.10)",
      "@ar-agents/mercadopago/otel subpath (OpenTelemetry) (NEW v0.10)",
      "confirm_3ds_challenge full flow + search_payments_all (NEW v0.10)",
      "CircuitBreaker + deadline propagation + W3C Trace Context (v0.9)",
      "mp_health_check + property-based tests + benchmarks (v0.9)",
      "Edge Runtime + Web Crypto + replay protection (v0.8)",
      "Vercel KV subpath: subscription/oauth/idempotency adapters (v0.8)",
      "Cookbook con 8 recipes copy-pasteables",
      "+ 77 más (86 total)",
    ],
