const PACKAGES = [
  ...
  },
  {
    ...
    version: "0.8.0",
    purpose:
      "EL agente de Mercado Pago más completo posible (81 tools, 100% del API público de MP). **v0.8 = production-ready architecture**: Edge Runtime support (Web Crypto, sin node:crypto — corre en Vercel Edge Functions, Cloudflare Workers, Deno), HMAC webhook verify con replay-attack protection (rechaza signatures > 5min de antiguedad), Vercel KV adapters drop-in (state, OAuth tokens, idempotency cache) via subpath @ar-agents/mercadopago/vercel-kv, y un cookbook completo con 8 recipes runnable: Checkout Pro / SaaS subscription con card swap / Webhook handler en Edge / Marketplace split / QR in-store / 3DS challenge / auth-only Order (ride-share) / recovery patterns. **Comparado con Stripe Agent Toolkit**: 81 tools vs 26, HMAC verify combo (Stripe no tiene), explain_payment_status helper (Stripe no tiene), self-contained bundle (Stripe es thin wrapper sobre mcp.stripe.com), AR-first.",
    tools: [
      "Edge Runtime + Web Crypto + replay protection (NEW v0.8)",
      "Vercel KV subpath: subscription/oauth/idempotency adapters (NEW v0.8)",
      "Cookbook con 8 recipes copy-pasteables (NEW v0.8)",
      "compute_marketplace_fee + explain_payment_status (pure helpers)",
      "list_point_devices + create_point_payment_intent (terminal hardware)",
      "handle_webhook (HMAC verify combo + 3DS analyzer)",
      "OAuth Marketplace flow (3 tools)",
      "create_order + capture_order + cancel_order (Order API con auth-only)",
      "create_subscription_plan + subscribe_to_plan + update_subscription (card swap)",
      "+ 70 más (81 total)",
    ],
