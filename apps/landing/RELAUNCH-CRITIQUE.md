...
4. **No proof above the fold; the first thing after the headline is a *warning* banner.** (conversion=high, design, my eval). Proof (live agent, "we incorporated ourselves", 37 packages/MIT) is all below the fold. → Pull one proof strip up; reframe the law banner from "not ready" to "early access / why now."
...
6. **All machine signals still sell the OLD positioning.** (technical=critical×2). `<title>`/meta/OG/JSON-LD say "open infrastructure for AI corporations / 37 packages"; the FAQPage + HowTo JSON-LD describe Q&A/steps no longer on the page (Google policy risk + AI engines quote the wrong product). → Rewrite all meta around the new promise; delete or rebuild+render the FAQ/HowTo schema.
...
11. **Copy nits:** "las rieles" → **"los rieles"** (riel is masculine); lock one canonical number (37 packages / 243 tools / 89 flagship are used inconsistently).
