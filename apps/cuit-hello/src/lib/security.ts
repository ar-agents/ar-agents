 ...
 *
 * cuit-hello is a single-region demo. In serverless, in-memory state is
 * per-instance — Vercel may run multiple concurrent instances, so a determined
 * attacker can get N×LIMIT requests by hammering many instances. That's
 * acceptable for a demo: the limit is mainly there to keep abusive bots from
 * burning through Anthropic credits. For production traffic, swap this for
 * `@upstash/ratelimit` (Vercel KV-backed).
 *
 * # Identification
 *
