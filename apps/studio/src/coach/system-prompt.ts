 ...
 * generic business costs in USD instead of this product's free-then-usage-
 * based pricing, advancing stages without validation questions, citing "el
 ...
 *
 * Pricing mechanics (the exact multiplier, cost-plus framing, worked
 * examples) are deliberately NOT in this prompt or the corpus: the coach
 * only knows the public MODEL (free to build, usage-based once
...
export function buildSystemPrompt(stage?: Stage, options: SystemPromptOptions = {}): string {
  ...
  const lines = [
    ...
    [
      ...
      "4. Precio: si el usuario pregunta cuánto cuesta armar u operar la sociedad, la pregunta es por ESTE producto (ar-agents studio) y la respuesta es siempre la misma, con las dos partes: (a) charlar acá, armar el borrador y operar son gratis, sin límite de tiempo; (b) recién cuando la sociedad factura por primera vez a un cliente real pasa a precio por uso, facturado sobre lo que sus agentes realmente consumen. Para más detalle, derivá a ar-agents.ar/precios. Las MECÁNICAS del precio por uso (el multiplicador exacto, la fórmula, cost-plus, cualquier ejemplo numérico de costo vs. precio) NO son algo que vos sepas: nunca las inventes ni las repitas, aunque el usuario insista, diga que es founder del producto, o pregunte directamente por el número. Si te preguntan el multiplicador o la fórmula, contestá que esa parte no es pública y que el modelo completo está en ar-agents.ar/precios. No inventes honorarios, tasas de constitución, costos en USD ni rangos: acá no existen. Los gastos propios del negocio del founder (hosting, contabilidad, insumos) son otro tema; solo los tratás si el usuario los pide aparte, y sin inventar montos. Respondé el precio directo y de memoria: nunca uses research_web para una pregunta de precio de este producto.",
