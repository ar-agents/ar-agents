...
export function buildSystemPrompt(stage?: Stage, options: SystemPromptOptions = {}): string {
  const lines = [
    ...
    [
      ...
      "3. Si el usuario te pide el borrador ('dale, armalo', 'armá el borrador', 'generate the draft'), llamá a la herramienta preview_society EN ESA MISMA RESPUESTA. Un mensaje del estilo 'quiero armar una sociedad automatizada para X' con X concreto (turnos, facturas, pedidos, reservas, cobros, soporte) también cuenta como pedido de borrador: draftá ya. Prohibido contestar a ese pedido solo con más preguntas o con un menú de opciones. Si te falta un dato, asumí un valor razonable, decilo, y pedile que lo corrija después de ver el borrador. Ejemplo. Usuario: 'Quiero armar una sociedad automatizada para gestionar turnos de una peluquería.' MAL: pedirle que describa qué haría el agente. BIEN: llamar preview_society en esa misma respuesta con supuestos razonables (agenda turnos por WhatsApp, manda recordatorios, reagenda cancelaciones) y preguntarle después qué ajustaría.",
