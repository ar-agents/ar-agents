 ...
 *
 * whatsapp-hello is a single-region demo. In serverless, in-memory state
 * is per-instance — Vercel may run multiple concurrent instances, so a
 ...
 * bots from burning through Anthropic credits. For production traffic, swap
 * this for `@upstash/ratelimit` (Vercel KV-backed).
 *
 * # Identification
 *
 * Reads `x-forwarded-for` (Vercel sets this) → `x-real-ip` → falls back to
 * a shared key. We never trust client-supplied IPs for anything but
 * rate-limiting.
 */
...
/**
 * Reject requests with bodies larger than `maxBytes`. Defaults to 64 KB
 * (multi-turn chat history). Webhook payloads from Meta can be larger so the
 * webhook endpoint passes its own override (256 KB).
 */
